...
<body>
  ...
  <nav class="tabs" aria-label="Settings sections"><button class="tab active" onclick="openTab('status')">Статус</button><button class="tab" onclick="openTab('runtime')">Runtime</button><button class="tab" onclick="openTab('gateway')">Gateway</button><button class="tab" onclick="openTab('secrets')">Секреты</button><button class="tab" onclick="openTab('security')">Security</button><button class="tab" onclick="openTab('prompts')">Промпты</button><button class="tab" onclick="openTab('memory')">Память</button></nav>
  <div class="grid">
    <section class="card active" data-tab="status"><h2>Статус</h2><button onclick="loadStatus()" class="secondary">Обновить</button><button onclick="voiceToggle()">Разбудить / усыпить</button><p class="muted">Использует тот же voice toggle, что и touch: если диалог спит — разбудит, если слушает — остановит.</p><pre id="status">...</pre></section>
